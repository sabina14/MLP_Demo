...
def get_callbacks(config, X_train):
    ...
    tensorboard_logs = os.path.join(logs["logs_dir"], logs["tensorboard_logs"], unique_dir_name)
    #tensorboard_logs=os.path.join(logs,logs_dir,tensorboard_logs,unique_dir_name)
    os.makedirs(tensorboard_logs, exist_ok=True)

    tensorboard_cb = tf.keras.callbacks.TensorBoard(log_dir=tensorboard_logs)

    file_writer = tf.summary.create_file_writer(logdir=tensorboard_logs)
